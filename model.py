...
class BigramModel:
    def __init__(
        self, tokens: list, bigram_table=None, unigram_table=None, from_json=False
    ):
        if tokens == None:
            print("Tokens cannot be null!")
        else:
            self.tokens: list = (
                BigramModel.add_sentence_boundaries(tokens) if not from_json else tokens
            )
            _unigram_counts = (
                BigramModel.make_count_unigrams(self.tokens) if not from_json else None
            )
            _unigram_counts_tuples = (
                _unigram_counts.most_common(len(_unigram_counts))
                if not from_json
                else None
            )
            self.unigram_frequency_table = (
                pd.DataFrame(_unigram_counts_tuples, columns=["unigram", "count"])
                if not from_json
                else unigram_table
            )
            self.unigram_frequency_table.drop(
                self.unigram_frequency_table[
                    self.unigram_frequency_table["unigram"] == "</s>"
                ].index,
                inplace=True,
            ) if not from_json else None
            _bigram_counts = (
                BigramModel.make_count_bigrams(self.tokens) if not from_json else None
            )
            _bigram_count_tuples = (
                _bigram_counts.most_common(len(_bigram_counts))
                if not from_json
                else None
            )
            self.bigram_frequency_table = (
                pd.DataFrame(_bigram_count_tuples, columns=["bigram", "count"])
                if not from_json
                else bigram_table
            )
    ...
    def __getstate__(self):
        dict = {
            "bigram_pickle": self.bigram_frequency_table.to_dict(),
            "unigram_pickle": self.unigram_frequency_table.to_dict(),
            "tokens": self.tokens,
        }
        return dict
    ...
    def probability(self, w: str, w_n: str, smoothing_constant: float = 0.0):
        ...
        bigram: tuple = (w, w_n)
        try:
            bigram_count = self.bigram_frequency_table.loc[
                self.bigram_frequency_table["bigram"] == bigram
            ]["count"].tolist()[0]
        except:
            return 0.0
        try:
            unigram_count = self.unigram_frequency_table.loc[
                self.unigram_frequency_table["unigram"] == w_n
            ]["count"].tolist()[0]
        except:
            return 0.0
    ...
    def choose_successor(
        self, word: str, smoothing_constant: float = 0.0
    ) -> str | None:
        pass
    ...
    def make_count_unigrams(tokens: list) -> Counter:
        ...
        for p, words in enumerate(
            tqdm(tokens, ncols=100, desc="Counting Unigrams")
    ...
    def add_sentence_boundaries(tokens: list) -> list:
        ...
        for i, item in enumerate(
            tqdm(tokens_without_punctuation, ncols=100, desc="Adding boundaries")
        ):  # tqdm prints a progressbar
            item.insert(0, "<s>")
            item.append("</s>")
