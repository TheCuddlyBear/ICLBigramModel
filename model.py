...
class BigramModel:
    ...
    def __init__(self,
        ...
        if tokens == None:
            ...
            self.unigram_frequency_table = (pd.DataFrame(
                _unigram_counts_tuples, columns=["unigram", "count"]) if
                                            not from_json else unigram_table)
            self.unigram_frequency_table.drop(
                self.unigram_frequency_table[
                    self.unigram_frequency_table["unigram"] == "</s>"].index,
                inplace=True,
            ) if not from_json else None
            ...
            self.bigram_frequency_table = (pd.DataFrame(
                _bigram_count_tuples, columns=["bigram", "count"])
                                           if not from_json else bigram_table)
    ...
    def probability(self, w: str, w_n: str, smoothing_constant: float = 0.0):
        ...
        try:
            bigram_count = self.bigram_frequency_table.loc[
                self.bigram_frequency_table["bigram"] ==
                bigram]["count"].tolist()[0]
        except:
            return 0.0
        try:
            unigram_count = self.unigram_frequency_table.loc[
                self.unigram_frequency_table["unigram"] ==
                w_n]["count"].tolist()[0]
        except:
            return 0.0
    ...
    def perplexity(self, sent: list, smoothing_constant: float = 1.0) -> float:
        ...
        probs = []
        for i in range(n - 1):
            probability = self.probability(sentCopy[i], sentCopy[i + 1],
                                           smoothing_constant)
            q = 1 / probability
            probs.append(q)
        s = math.prod(probs)
    ...
    def choose_successor(self,
                         word: str,
                         smoothing_constant: float = 0.0) -> str | None:
        pass
    ...
    def make_count_unigrams(tokens: list) -> Counter:
        ...
        for p, words in enumerate(
                tqdm(tokens, ncols=100,
                     desc="Counting Unigrams")):  # tqdm prints a progressbar
            for word in words:
    ...
    def make_count_bigrams(tokens: list) -> Counter:
        ...
        for p, words in enumerate(
                tqdm(tokens, ncols=100, desc="Counting Bigrams")):
    ...
    def add_sentence_boundaries(tokens: list) -> list:
        ...
        for i, item in enumerate(
                tqdm(tokens_without_punctuation,
                     ncols=100,
                     desc="Adding boundaries")):  # tqdm prints a progressbar
            item.insert(0, "<s>")
            item.append("</s>")
    ...
    def remove_punctuation_tokens(tokens: list):
        ...
        for sent in tokens:
            to_return.append(
                [p.lower() for p in sent if not re.match("\W", p)])
        return to_return
